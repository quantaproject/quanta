...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " quanta-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The quanta-tx tool is used for creating and modifying quanta transactions.\n\n"
                "quanta-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded quanta transaction, or with \"-create [commands]\" to create a hex-encoded bitcoin transaction.\n"
                "\n"
                "Usage: quanta-tx [options] <hex-tx> [commands]\n"
                "or:    quanta-tx [options] -create [commands]\n"
                "\n";
